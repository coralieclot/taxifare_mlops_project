...
def preprocess(min_date:str = '2009-01-01', max_date:str = '2015-01-01') -> None:
    ...
    # Local paths for saving raw and processed data
    data_query_cache_path = Path(LOCAL_DATA_PATH).joinpath("raw", f"query_{min_date}_{max_date}_{DATA_SIZE}.csv")

    #Get raw data
    data = get_data_with_cache(GCP_PROJECT,
                               query,
    ...
    # Concatenate pick_datetime, X_processed and y to create data_processed
    ...
    load_data_to_bq(data_processed,
                    GCP_PROJECT,
                    BQ_DATASET,
                    f'processed_{DATA_SIZE}',
                    truncate = True)
...
def train(
    ...
    processed_data = get_data_with_cache(
        GCP_PROJECT,
        query,
        data_processed_path,
    ...
    train_length = int(len(processed_data) * (1 - split_ratio))

    data_train = processed_data.iloc[:train_length, :].sample(frac=1).to_numpy()
    data_val = processed_data.iloc[train_length:, :].sample(frac=1).to_numpy()
